...
4. File System Monitoring: Using inotify for real-time file change detection
5. Terminal UI: Interactive dashboard with live updates
6. Signal Handling: Graceful shutdown and cleanup
...
3. DNS Cache Poisoning Detection:
   - Monitors /etc/hosts for unauthorized modifications
   - Uses Linux inotify subsystem for real-time file watching
   - Detects malicious DNS redirections
   - Tracks file modification timestamps and content changes
...
HOSTS_FILE = "/etc/hosts"  # DNS cache file to monitor
...
class DNSCachePoisoningDetector:
    """
    Detects unauthorized modifications to /etc/hosts file

    OS Concept: The Linux inotify subsystem provides file system event monitoring:
    ...
    - Malicious entries can redirect traffic
    - Requires root access to modify
    - Critical for phishing and MITM attacks
    """

    def __init__(self, hosts_file: str = HOSTS_FILE):
        self.hosts_file = hosts_file
        self.initial_hash = self._calculate_hash()
        self.initial_content = self._read_hosts_file()
        ...
        # Start inotify watcher if available
    ...
    def _calculate_hash(self) -> Optional[str]:
        """Calculate SHA256 hash of hosts file"""
        try:
            with open(self.hosts_file, 'rb') as f:
    ...
    def _start_inotify_watcher(self):
        """
        Start inotify file watcher in background thread

        OS Concept: inotify is a Linux kernel subsystem that monitors
        file system events. It's more efficient than polling because:
        - Events are generated by kernel
        - No CPU wasted on polling
        - Immediate notification on changes
        - Can watch multiple files/directories
        """
        def watcher_thread():
            try:
                ...
                wm.add_watch(self.hosts_file, mask)
    ...
    def _on_file_modified(self, event):
        """Handle file modification event from inotify"""
        with self.alert_lock:
            current_hash = self._calculate_hash()
            if current_hash and current_hash != self.initial_hash:
                ...
                if current_content:
                    analysis = self._analyze_modification(self.initial_content, current_content)

                    alert = Alert(
                        severity="CRITICAL",
                        alert_type="DNS_POISONING",
                        message=f"/etc/hosts modified! {analysis['summary']}",
                        mitigation="Restore from backup and audit root privilege escalation"
                    )
                    self.pending_alerts.append(alert)

    def _analyze_modification(self, original: Optional[str], current: Optional[str]) -> Dict:
        """
        Analyze what changed in /etc/hosts

        Returns dict with:
        - summary: Brief description of changes
        - added_entries: New malicious entries
        - removed_entries: Removed entries
        """
        if not original or not current:
            return {"summary": "Content changed", "added_entries": [], "removed_entries": []}
        ...
        added_entries = [line for line in added if line.strip() and not line.strip().startswith('#')]
        removed_entries = [line for line in removed if line.strip() and not line.strip().startswith('#')]

        if added_entries:
            summary = f"Added {len(added_entries)} entry(ies)"
        elif removed_entries:
            summary = f"Removed {len(removed_entries)} entry(ies)"
        else:
            summary = "File metadata changed"
    ...
    def check_for_modifications(self) -> List[Alert]:
        """
        Check for /etc/hosts modifications

        This is called periodically by the detection engine.
        If inotify is active, it returns pending alerts from the watcher thread.
        If inotify is not available, it polls the file for changes.
        """
        alerts = []

        with self.alert_lock:
            # Return any pending alerts from inotify
            if self.pending_alerts:
                alerts = self.pending_alerts.copy()
                self.pending_alerts.clear()

        # Fallback: Poll for changes if inotify not available
        if not INOTIFY_AVAILABLE or getattr(self, 'fallback_mode', False):
            current_hash = self._calculate_hash()
            if current_hash and current_hash != self.initial_hash:
                if not self.modification_detected:
                    self.modification_detected = True
                    self.last_modification_time = datetime.now()
    ...
    def get_modification_status(self) -> Dict:
        ...
        return {
            'modified': self.modification_detected,
            'last_modification': self.last_modification_time,
            'monitoring_method': 'inotify' if (INOTIFY_AVAILABLE and not getattr(self, 'fallback_mode', False)) else 'polling',
...
class SecurityDashboard:
    ...
    def __init__(self, alert_manager: AlertManager, system_monitor: SystemMonitor,
                 ...
                 dns_detector: DNSCachePoisoningDetector):
    ...
    def generate_status_panel(self) -> Panel:
        ...
        dns_status = self.dns_detector.get_modification_status()
        ...
        # DNS poisoning status
        dns_modified = dns_status['modified']
        dns_style = "bold red" if dns_modified else "green"
        dns_indicator = "⚠️  POISONED" if dns_modified else "✓ Clean"
        table.add_row("/etc/hosts", f"[{dns_style}]{dns_indicator}[/]")
...
class DetectionEngine:
    ...
    def __init__(self):
        ...
        self.dns_detector = DNSCachePoisoningDetector()
    ...
    def run_detection_cycle(self):
        ...
        dns_alerts = self.dns_detector.check_for_modifications()
...
def print_startup_banner():
    ...
    console = Console()
    console.print("\n[bold cyan]" + "=" * 70 + "[/]")
    console.print("[bold cyan]  Security Vulnerability Detection Framework - Phase 4[/]")
    ...
    console.print("[cyan]  → DNS Cache Poisoning: Watching /etc/hosts with inotify[/]")
    console.print("[cyan]  → Dashboard UI: Initializing real-time display[/]\n")

    if not INOTIFY_AVAILABLE:
        console.print("[yellow]  ⚠️  pyinotify not available - using polling mode for DNS detection[/]")
        console.print("[yellow]     Install with: pip3 install pyinotify[/]\n")
